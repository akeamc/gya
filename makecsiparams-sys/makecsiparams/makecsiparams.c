...
void usage()
{
    char *usage_str =
        ...
        "   ~~-r           generate raw output (no base64)~~ [ALWAYS ENABLED]\n"
        "";
    fprintf(stdout, "%s\n", usage_str);
}
...
int cli(int argc, char *argv[], uint8_t* buf)
{
    struct csi_params p;
    memset(&p, 0, sizeof(p));
    int c;
    ...
    while ((c = getopt(argc, argv, "he:m:b:c:C:N:d:")) != EOF)
    {
        switch (c)
        {
        case 'h':
